...
if not KnoxLife or (KnoxLife.API_VERSION or 0) < 1 then
    print("[KnoxLifeBobcats] KnoxLife not found (or too old); "
...
if AnimalDefinitions and AnimalDefinitions.animals
    ...
    print("[KnoxLifeBobcats] no animal definition for kwc_bobcatfemale; not registering. "
        .. "Its Definitions/animal file did not load.")
    return
end

-- The animation set and action group this mod ships under
--     42/media/AnimSets/kwc_bobcat/  and  42/media/actiongroups/kwc_bobcat/
--
...
if KW.registerAnimSet then
    KW.registerAnimSet("kwc_bobcat", {
        animset  = "kwc_bobcat",
        fallback = "raccoon",
        stages   = { "kwc_bobcatkitten", "kwc_bobcatfemale", "kwc_bobcatmale" },
...
if KW.registerSpecies(ID, {
    ...
    babyChance = 35,
    -- ⚠️ A PREDATOR MUST OUTPACE ITS PREY. This was 0.07, against a rabbit's
    ...
    trackSize = "medium", speed = 0.16,
    density = DENSITY,
    habitat = ID,
    enabledOption = "KnoxLifeBobcats.Enabled",
    routeOption  = "KnoxLifeBobcats.Routes",
    groupOption  = "KnoxLifeBobcats.GroupSize",
}) then
    ...
    print("[KnoxLifeBobcats] registered on its own habitat (API v" .. KW.API_VERSION .. ")")
end
